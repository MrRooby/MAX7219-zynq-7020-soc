...
always @(posedge S_AXI_ACLK or negedge S_AXI_ARESETN) begin
	if(S_AXI_ARESETN) begin
		...
		BFM_R_IDLE: begin
			if(R_RQ) begin
				$display("%m:%6t - READ transaction start @0x%0h.", $time, W_ADDR);
				R_STATE = BFM_R_ADDR;
				R_CNT = 0;
				S_AXI_ARADDR = R_ADDR;
				S_AXI_ARVALID = 1'b1;
		...
		BFM_R_ADDR: begin
			if(S_AXI_ARREADY == 1'b1) begin
				$display("%m:%6t : READ transaction - address ACK (%d).", $time, W_CNT);
				S_AXI_ARVALID = 1'b0;
				...
				if(S_AXI_RVALID) begin
					...
					R_RESP = S_AXI_RRESP;
					S_AXI_RREADY = 1'b0;
					$display("%m:%6t : READ transaction - data ACK (%d).", $time, W_CNT);
